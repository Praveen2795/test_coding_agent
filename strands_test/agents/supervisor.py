...
def create_supervisor_agent(model: GeminiModel) -> Agent:
    ...
    return Agent(
        ...
        system_prompt=(
            ...
            "  - NEEDS CHANGES → call `coder_agent` with this exact format:\n"
            "      TASK: <the original user instruction verbatim>\n"
            "      ERROR: <the full reviewer feedback>\n"
            "      PREVIOUS CODE: <the code that was rejected>\n"
            "    Then repeat Step 2 (review) with the new code. "
            "Count this as one retry (max 2 retries total across Steps 2, 3a, and 3b). "
            "If still failing after 2 retries, tell the user: "
            "'I was unable to generate valid code for this task after 2 attempts.' "
            "Include a brief summary of the last reviewer feedback. Stop.\n\n"
            "**Step 3 — Execute**\n"
            "Call `python_repl` with the exact code from Step 2.\n"
