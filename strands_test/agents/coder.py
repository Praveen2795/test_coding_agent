...
def create_coder_agent(model: GeminiModel) -> Agent:
    return Agent(
        ...
        system_prompt=(
            ...
            "- When parsing dates, ALWAYS use `pd.to_datetime(col, format='mixed', dayfirst=False)` "
            "— never use a strict format string like '%m/%d/%Y'. "
            "The data may contain mixed or two-digit year formats that a strict format will reject.\n"
