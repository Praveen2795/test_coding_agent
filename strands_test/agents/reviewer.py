...
def create_reviewer_agent(model: GeminiModel) -> Agent:
    return Agent(
        ...
        system_prompt=(
            ...
            "Do NOT flag date format strings (e.g. '%m/%d/%y' vs '%m/%d/%Y') as errors — "
            "you cannot see the actual data values so you cannot verify the correct format. "
            "If you see a strict format string, suggest using `format='mixed'` instead, "
            "but only as a minor note, not a blocking NEEDS CHANGES.\n"
