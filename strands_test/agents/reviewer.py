...
def create_reviewer_agent(model: GeminiModel) -> Agent:
    return Agent(
        ...
        system_prompt=(
            ...
            "   e.g. if the task asks for unique accounts, does it use distinct accountnumber (not account_id)?\n"
            "2) Logic errors — wrong aggregations, incorrect filters, off-by-one, wrong columns used.\n"
            "3) API usage — correct endpoint, correct response key ('data', 'accounts', 'tags'), correct params.\n"
